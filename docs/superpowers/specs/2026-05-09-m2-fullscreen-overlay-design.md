...
### `iced_layershell` API deviations from plan

**Version pinned:** `iced_layershell = "0.13.7"` (targets `iced ^0.13`). Also required adding `tokio` feature `macros` for `#[tokio::main]`.

**`Application::run` asserts against `StartMode::AllScreens`** — this mode is only valid with the `daemon` build pattern. Using `Application::run` with `AllScreens` panics at runtime. Fix: switch to `build_pattern::daemon`.

**`#[to_layer_message]` vs `#[to_layer_message(multi)]`** — the plain `#[to_layer_message]` macro generates `TryInto<LayershellCustomActions>` (for single-window `Application`). The `daemon` build pattern requires `TryInto<LayershellCustomActionsWithId>`, which is only generated by `#[to_layer_message(multi)]`.

**View closure lifetime** — passing a closure as the `view` argument to `daemon()` fails to satisfy the `for<'a> View<'a, ...>` HRTB bound. Fix: extract view logic into a named function `fn overlay_view(state: &OverlayState, window: iced::window::Id) -> Element<'_, ...>`.

**`MainSettings` used instead of `Settings<Flags>`** — the daemon pattern takes `MainSettings` (no `flags` field); initial state is passed via `run_with(|| (State, Task))`.

### Multi-output behaviour

`StartMode::AllScreens` creates one surface per output, but M2 shows the same (first) frame on all surfaces. Per-output frame mapping is deferred to M3.
